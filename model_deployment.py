...
if select_page == 'Analysis':
    ...
    def main():
        ...
        tab2.write('### Does occupation affects claim flag ?')
        top_occ = cleaned_df.occupation.value_counts()
        tab2.plotly_chart(px.histogram(top_occ, x= top_occ.index, y= top_occ.values))
        
        #num_cols = cleaned_df.select_dtypes(exclude= 'O')
        #tab3.plotly_chart(px.imshow(cleaned_df.corr(), x= num_cols))
